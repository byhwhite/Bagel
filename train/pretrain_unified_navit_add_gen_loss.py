...
class DatasetFirstCycleTracker:
    def __init__(self, expected_datasets):
        self.expected_datasets = set(expected_datasets)
        self.first_index_by_worker = {}  # (dataset_name, worker_id) -> first data index
        self.seen_workers = {}           # dataset_name -> set(worker_id)
        self.wrapped_workers = {}        # dataset_name -> set(worker_id)

    @staticmethod
    def _normalize_index(sample_data_index):
        data_index = sample_data_index.get("data_indexes")
    ...
    def update(self, batch_data_indexes):
        if not batch_data_indexes:
            return False

        for sample_data_index in batch_data_indexes:
            dataset_name = sample_data_index.get("dataset_name")
            if dataset_name not in self.expected_datasets:
                continue

            worker_id = sample_data_index.get("worker_id", -1)
            key = (dataset_name, worker_id)
            normalized_index = self._normalize_index(sample_data_index)
            first_index = self.first_index_by_worker.get(key)

            if dataset_name not in self.seen_workers:
                self.seen_workers[dataset_name] = set()
            if dataset_name not in self.wrapped_workers:
                self.wrapped_workers[dataset_name] = set()
            self.seen_workers[dataset_name].add(worker_id)

            if first_index is None:
                self.first_index_by_worker[key] = normalized_index
            elif normalized_index == first_index:
                self.wrapped_workers[dataset_name].add(worker_id)

        # A dataset is considered completed when all seen workers for this dataset wrapped once.
        for dataset_name in self.expected_datasets:
            if dataset_name not in self.seen_workers or len(self.seen_workers[dataset_name]) == 0:
                return False
            if not self.seen_workers[dataset_name].issubset(self.wrapped_workers[dataset_name]):
                return False
        return True
...
class TrainingArguments:
    ...
    stop_on_dataset_first_cycle: bool = field(
        default=False,
        metadata={"help": "Stop when all active dataset groups complete their first observed cycle."}
    )
    ...
    use_ema: bool = field(
        default=True,
        metadata={"help": "Whether to maintain and save an EMA copy of model weights."}
    )
    ...
    snr_gamma: float = field(
        default=0.0,
        metadata={"help": "If > 0, apply Min-SNR weighting to visual-gen MSE loss."}
    )
    snr_eps: float = field(
        default=1e-6,
        metadata={"help": "Numerical epsilon used in SNR weighting."}
    )
    perceptual_weight: float = field(
        default=0.0,
        metadata={"help": "Scaling factor for perceptual loss on decoded images (0 disables)."}
    )
...
def main():
    ...
    use_perceptual = training_args.visual_gen and training_args.perceptual_weight > 0
    perceptual_backbone = None
    if use_perceptual:
        try:
        ...
        perceptual_backbone = vgg.to(device).eval()
        for param in perceptual_backbone.parameters():
            param.requires_grad = False
    ...
    # def print_frozen_and_trainable_params(model, logger, max_print=200):
    #     frozen = []
    #     trainable = []
    ...
    #     logger.info(f"Trainable param tensors: {len(trainable)}")
    #     logger.info(f"Frozen param tensors: {len(frozen)}")

    #     for n in frozen[:max_print]:
    #         logger.info(f"[FROZEN] {n}")
    #     if len(frozen) > max_print:
    #         logger.info(f"... ({len(frozen) - max_print} more frozen params)")


    # def find_mixed_requires_grad_modules_recursive(model, logger):
    ...
    #         n_train = sum(flags)
    #         n_frozen = len(flags) - n_train
    #         if n_train > 0 and n_frozen > 0:
    #             logger.info(
    #                 f"[MIXED] {name} ({module.__class__.__name__}): "
    #                 f"trainable={n_train}, frozen={n_frozen}"
    #             )


    # def summarize_top_modules(model, logger):
    #     for name, module in model.named_children():
    #         params = list(module.parameters())
    #         if not params:
    #             continue
    #         n_total = len(params)
    #         n_train = sum(p.requires_grad for p in params)
    #         n_frozen = n_total - n_train
    #         logger.info(
    #             f"[TOP] {name}: total={n_total}, trainable={n_train}, frozen={n_frozen}"
    #         )

    # print_frozen_and_trainable_params(model, logger)
    # find_mixed_requires_grad_modules_recursive(model, logger)
    # summarize_top_modules(model, logger)
    ...
    active_grouped_datasets = []
    for grouped_name, grouped_args in dataset_meta.items():
        weight = float(grouped_args.get("weight", 0.0))
        is_mandatory = bool(grouped_args.get("is_mandatory", False))
        if is_mandatory or weight > 0.0:
            active_grouped_datasets.append(grouped_name)
    ...
    dataset_cycle_tracker = DatasetFirstCycleTracker(active_grouped_datasets)
    if dist.get_rank() == 0:
        if training_args.stop_on_dataset_first_cycle:
            logger.info(
                f"Dataset-cycle stop enabled for active groups: {sorted(active_grouped_datasets)}"
            )
        else:
            logger.info("Dataset-cycle stop disabled. Training will stop by total_steps.")
    ...
    for micro_step, data in enumerate(train_loader):
        ...
        if training_args.stop_on_dataset_first_cycle:
            local_cycle_done = torch.tensor(
                int(dataset_cycle_tracker.update(data_indexes)),
                device=device,
            )
            dist.all_reduce(local_cycle_done, op=dist.ReduceOp.MIN)
            if local_cycle_done.item() == 1:
                logger.info(
                    f"All active dataset groups finished first cycle. Stopping at step={curr_step}."
                )
                break
        ...
        if training_args.visual_gen:
            ...
            mse_token = mse.mean(dim=-1)
            if training_args.snr_gamma > 0:
                timesteps = loss_dict["mse_timesteps"].clamp(min=training_args.snr_eps, max=1 - training_args.snr_eps)
                snr = ((1.0 - timesteps) ** 2) / (timesteps ** 2 + training_args.snr_eps)
                snr_weights = torch.clamp(training_args.snr_gamma / (snr + training_args.snr_eps), max=1.0)
                mse = (mse_token * snr_weights).sum() * dist.get_world_size() / total_mse_tokens
            else:
                mse = mse_token.sum() * dist.get_world_size() / total_mse_tokens
            loss_dict["mse"] = mse.detach()
            loss = loss + mse * training_args.mse_weight
            if use_perceptual:
                pred_flow = loss_dict["pred_flow"]
                target_flow = loss_dict["target_flow"]
                pred_latent = target_flow + pred_flow
                pred_latent = pred_latent.reshape(-1, model.latent_patch_size, model.latent_patch_size, model.latent_channel)
                pred_latent = pred_latent.reshape(-1, model.latent_patch_size * model.latent_patch_size * model.latent_channel)
                target_latent = target_flow.reshape_as(pred_flow)
                latent_token_count = pred_latent.shape[0]
                side = int(latent_token_count ** 0.5)
                # if side * side == latent_token_count and side > 0:
                pred_latent_img = pred_latent.view(1, side, side, model.latent_patch_size, model.latent_patch_size, model.latent_channel)
                pred_latent_img = pred_latent_img.permute(0, 5, 1, 3, 2, 4).reshape(1, model.latent_channel, side * model.latent_patch_size, side * model.latent_patch_size)
                target_latent_img = target_latent.view(1, side, side, model.latent_patch_size, model.latent_patch_size, model.latent_channel)
                target_latent_img = target_latent_img.permute(0, 5, 1, 3, 2, 4).reshape(1, model.latent_channel, side * model.latent_patch_size, side * model.latent_patch_size)
                pred_img = torch.clamp(vae_model.decode(pred_latent_img), -1, 1)
                target_img = torch.clamp(vae_model.decode(target_latent_img), -1, 1)
                pred_feat = perceptual_backbone((pred_img + 1) / 2)
                with torch.no_grad():
                    target_feat = perceptual_backbone((target_img + 1) / 2)
                p_loss = F.l1_loss(pred_feat, target_feat)
                loss = loss + p_loss * training_args.perceptual_weight
                loss_dict["perceptual"] = p_loss.detach()
                # else:
                #     loss_dict["perceptual"] = torch.tensor(0.0, device=device)
        else:
